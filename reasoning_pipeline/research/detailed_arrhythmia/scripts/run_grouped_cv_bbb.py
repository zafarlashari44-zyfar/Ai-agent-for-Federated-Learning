...
from research.detailed_arrhythmia.dataset.ontology_bbb import BBB_ONTOLOGY
...
def evaluate_fold(
    ...
    present_labels = [
        label
        for label in BBB_ONTOLOGY.labels
    ...
    normal_index = BBB_ONTOLOGY.labels.index("N")
...
def train_fold(
    ...
    result = {
        "experiment": "bbb_ontology_ablation",
...
def main() -> None:
    ...
    print(
        "Weighted cross entropy unchanged."
    )
    ...
    for fold in manifest["folds"]:
        ...
        validation_counts = np.bincount(
            validation_labels,
            minlength=len(BBB_ONTOLOGY.labels),
        )
    ...
    summary = {
        "experiment": (
            "grouped_patient_cv_bbb_ontology"
        ),
